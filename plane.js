;window.onload = function(){
	...
	// 声明需要使用到的全局变量
	var gameStatus = false // 当前的游戏状态(没开始)
	,	a = null // 创建子弹的定时器
	,	b = null // 创建敌机的定时器
	,	c = null // 背景图运动的定时器
	...
	gameStart.firstElementChild.onclick = function(){
		...
		// 给当前的文档添加键盘事件
		document.onkeyup = function(evt){//按下后实现游戏开始
			var e = evt || window.event;
			// 获取到键盘的键值
			var keyVal = e.keyCode;
			if(keyVal == 32){//空格键键值32
				if(!gameStatus){
					// 初始化得分
					scores = 0;
					// 开始游戏
	...
	function myPlaneMove(evt){
		var e = evt || window.event;
		// 获取鼠标移动时的位置
		var mouse_x = e.x || e.pageX //||是或者
		,	mouse_y = e.y || e.pageY;

		// 计算得到鼠标移动时己方飞机的左上边距
		var last_myPlane_left = mouse_x - gameML - myPlaneW/2//飞机宽度除2
		,	last_myPlane_top = mouse_y - gameMT/*游戏界面上外边距 */ - myPlaneH/2;
		// 控制飞机不能脱离当前的游戏界面
	...
	function createBullet(){
		...
		bulletsP.appendChild(bullet);//把子弹追加到父元素中
		bullets.push(bullet);
		move(bullet,"top");
	}
	// 子弹的运动:运动函数(匀速运动)
	function move(ele,attr){
		...
		ele.timer = setInterval(function(){
			...
			if(moveVal <= -bulletH){
				...
				ele.style[attr] = moveVal + speed + "px";//写进运动样式
			}
		},10);
	}
	// 创建敌机数据对象
	...
	function gameOver(){
		for(var i=0;i<enemys.length;i++){
			if(!enemys[i].dead){ // 游戏机界面内存活的敌机
				// 检测碰撞
				// 1、获取敌机的左上边距
				var enemyL = getStyle(enemys[i],"left")//获取左边距
				,	enemyT = getStyle(enemys[i],"top");//获取上边距
				// 2、获取敌机的宽高
